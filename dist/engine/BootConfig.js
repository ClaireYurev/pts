...
export class BootConfigManager {
    ...
    static parse() {
        ...
        if (params.has("level"))
            cfg.level = safeParseInt(params.get("level"), 1, 999);
        // Player State
        if (params.has("playerX"))
            cfg.playerX = safeParseFloat(params.get("playerX"), -10000, 10000);
        if (params.has("playerY"))
            cfg.playerY = safeParseFloat(params.get("playerY"), -10000, 10000);
        if (params.has("health"))
            cfg.health = safeParseInt(params.get("health"), 0, 9999);
        if (params.has("lives"))
            cfg.lives = safeParseInt(params.get("lives"), 0, 99);
        if (params.has("score"))
            cfg.score = safeParseInt(params.get("score"), 0, 999999);
        // Game Settings
        if (params.has("time"))
            cfg.time = safeParseInt(params.get("time"), 0, 999999);
        ...
        if (params.has("musicVolume"))
            cfg.musicVolume = safeParseFloat(params.get("musicVolume"), 0, 1);
        if (params.has("sfxVolume"))
            cfg.sfxVolume = safeParseFloat(params.get("sfxVolume"), 0, 1);
        if (params.has("mute"))
        ...
        if (params.has("cheats")) {
            ...
            if (cheatsParam) {
                cfg.cheats = cheatsParam.split(",")
                    ...
                    .filter(cheat => this.KNOWN_CHEATS.includes(cheat));
            }
        }
