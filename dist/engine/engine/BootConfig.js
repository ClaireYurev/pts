...
export class BootConfigManager {
    /**
     * Parse URL parameters into a BootConfig object
     */
    static parse() {
        ...
        // Game Pack & Level
        if (params.has("pack"))
            cfg.packUrl = params.get("pack");
        if (params.has("level"))
            cfg.level = parseInt(params.get("level"));
        // Player State
        if (params.has("playerX"))
            cfg.playerX = parseFloat(params.get("playerX"));
        if (params.has("playerY"))
            cfg.playerY = parseFloat(params.get("playerY"));
        if (params.has("health"))
            cfg.health = parseInt(params.get("health"));
        if (params.has("lives"))
            cfg.lives = parseInt(params.get("lives"));
        if (params.has("score"))
            cfg.score = parseInt(params.get("score"));
        // Game Settings
        if (params.has("time"))
            cfg.time = parseInt(params.get("time"));
        if (params.has("difficulty"))
            cfg.difficulty = params.get("difficulty");
        if (params.has("skipCutscene"))
            cfg.skipCutscene = params.get("skipCutscene") === "1";
        // Audio Settings
        if (params.has("musicVolume"))
            cfg.musicVolume = parseFloat(params.get("musicVolume"));
        if (params.has("sfxVolume"))
            cfg.sfxVolume = parseFloat(params.get("sfxVolume"));
        if (params.has("mute"))
            cfg.mute = params.get("mute") === "true";
        // Display Settings
        ...
        if (params.has("fullscreen"))
            cfg.fullscreen = params.get("fullscreen") === "true";
        if (params.has("platform"))
            cfg.platform = params.get("platform");
        // Input Settings
        if (params.has("inputMethod"))
            cfg.inputMethod = params.get("inputMethod");
        // Accessibility
        if (params.has("subtitles"))
            cfg.subtitles = params.get("subtitles") === "true";
        if (params.has("highContrast"))
            cfg.highContrast = params.get("highContrast") === "true";
        if (params.has("lang"))
            cfg.lang = params.get("lang");
        // Debug & Development
        if (params.has("debug"))
            cfg.debug = params.get("debug") === "true";
        if (params.has("cheats")) {
            const cheatsParam = params.get("cheats");
            cfg.cheats = cheatsParam.split(",").map(c => c.trim()).filter(c => c.length > 0);
        }
    ...
    static validate(cfg) {
        ...
        // Validate cheats
        if (cfg.cheats) {
            const invalidCheats = cfg.cheats.filter(cheat => !this.KNOWN_CHEATS.includes(cheat));
    ...
    static apply(cfg, engine) {
        try {
            ...
            if (player) {
                if (cfg.playerX !== undefined)
                    player.position.x = cfg.playerX;
                if (cfg.playerY !== undefined)
                    player.position.y = cfg.playerY;
                if (cfg.health !== undefined)
                    player.health = Math.max(0, Math.min(9999, cfg.health));
     ...
     */
    static generateShareUrl(engine) {
     ...
     */
    static async copyShareUrl(engine) {
        try {
...
BootConfigManager.KNOWN_CHEATS = Object.values(CheatFlag);
BootConfigManager.DIFFICULTY_LEVELS = ['easy', 'normal', 'hard', 'extreme'];
BootConfigManager.INPUT_METHODS = ['keyboard', 'gamepad', 'touch'];
BootConfigManager.SUPPORTED_LANGUAGES = ['en', 'es', 'fr', 'de', 'ja', 'zh'];
