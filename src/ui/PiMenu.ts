...
export class PiMenu {
    ...
    public setDevTools(cheatManager: CheatManager, freeCamera: FreeCamera, debugOverlay: DebugOverlay, engine: GameEngine): void {
    ...
    private async updateLibraryDisplay(): Promise<void> {
    ...
    private updatePacksList(containerId: string, items: LibraryItem[]): void {
    ...
    private createPackElement(item: LibraryItem): string {
    ...
    private createPiButton(): void {
        ...
        this.eventListeners.push(
            { element: piButton, type: 'mouseenter', listener: mouseEnterHandler },
            { element: piButton, type: 'mouseleave', listener: mouseLeaveHandler },
            { element: piButton, type: 'click', listener: clickHandler }
        );
    ...
    private createLibraryMenu(): void {
    ...
    private setupSaveMenuEvents(): void {
        // Back button
        ...
        document.querySelectorAll("#saveMenu .save-action-btn").forEach(btn => {
            const actionHandler = (e: Event) => {
                e.stopPropagation();
                const slot = Number((btn as HTMLElement).dataset.slot) - 1; // Convert to 0-based index
    ...
    private setupSettingsMenuEvents(): void {
        // Back button
    ...
    private setupPackActionHandlers(): void {
    ...
    private async handlePackPlay(packId: string): Promise<void> {
        if (!this.libraryManager) return;
    ...
    private async handlePackInstall(packId: string): Promise<void> {
        if (!this.libraryManager) return;
    ...
    private async handleSaveAction(slot: number, action: 'save' | 'load' | 'overwrite'): Promise<void> {
        try {
            if (!this.saveSystem) {
    ...
    private async performSave(slot: number): Promise<void> {
    ...
    private async performLoad(slot: number): Promise<void> {
        if (!this.saveSystem || !this.engine) {
            console.error('SaveSystem or Engine not available');
            return;
        }
    ...
    private async captureGameState(): Promise<Omit<SaveGameV1, 'slot' | 'timestamp'>> {
        if (!this.engine) {
    ...
    private async updateSaveSlotsDisplay(): Promise<void> {
        ...
        try {
            ...
            for (let i = 0; i < 3; i++) {
                ...
                const slotElement = document.querySelector(`[data-slot="${slot + 1}"]`) as HTMLElement;
    ...
    private handleSaveSlot(slot: number): void {
        try {
    ...
    private handlePackSelection(packFile: string): void {
    ...
    private applyDevCheats(): void {
        if (!this.cheatManager) return;

        const godElement = document.getElementById("devCheatGod") as HTMLInputElement;
        const noclipElement = document.getElementById("devCheatNoClip") as HTMLInputElement;
        const infTimeElement = document.getElementById("devCheatInfTime") as HTMLInputElement;
        const giveSwordElement = document.getElementById("devCheatGiveSword") as HTMLInputElement;
        const healthElement = document.getElementById("devCheatHealth") as HTMLInputElement;
    ...
    private updateDevActiveCheatsDisplay(): void {
        if (!this.cheatManager) return;
    ...
    private syncDevCheckboxes(): void {
        if (!this.cheatManager || !this.debugOverlay || !this.freeCamera) return;
        ...
        const elements = {
            ...
            infTime: document.getElementById("devCheatInfTime") as HTMLInputElement,
            giveSword: document.getElementById("devCheatGiveSword") as HTMLInputElement,
            health: document.getElementById("devCheatHealth") as HTMLInputElement,
            debugOverlay: document.getElementById("devDebugOverlay") as HTMLInputElement,
            freeCamera: document.getElementById("devFreeCamera") as HTMLInputElement
        };
    ...
    private loadGame(saveData: SaveData): void {
    ...
    private showMenu(menuName: string): void {
        try {
            // Validate menu name
    ...
    private setupKeyboardShortcuts(): void {
        ...
        this.eventListeners.push({ element: window, type: 'keydown', listener: keydownHandler as (e: Event) => void });
    }
    ...
    private async copyShareUrl(): Promise<boolean> {
        if (!this.engine) {
    ...
    private setupControlsEvents(): void {
        ...
        const keyboardPreset = document.getElementById('keyboardPreset') as HTMLSelectElement;
        if (keyboardPreset) {
            const presetHandler = () => {
    ...
    private startRebinding(action: string, device: 'keyboard' | 'gamepad'): void {
        ...
        try {
            ...
            this.engine.getInputMap().startRebind(action as 'Left' | 'Right' | 'Up' | 'Down' | 'Jump' | 'Action' | 'Block' | 'Pause', device);
        } catch (error) {
    ...
    private updateDevMenu(): void {
        ...
        const elements = {
            god: document.getElementById('devGod') as HTMLInputElement,
            noclip: document.getElementById('devNoclip') as HTMLInputElement,
            infTime: document.getElementById('devInfTime') as HTMLInputElement,
            giveSword: document.getElementById('devGiveSword') as HTMLInputElement,
            health: document.getElementById('devHealth') as HTMLInputElement,
            debugOverlay: document.getElementById('devDebugOverlay') as HTMLInputElement,
            freeCamera: document.getElementById('devFreeCamera') as HTMLInputElement
        };
