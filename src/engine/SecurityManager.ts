...
export class SecurityManager {
     ...
     */
    public checkRateLimit(action: string, identifier?: string): { allowed: boolean; remaining: number; resetTime: number } {
        if (!this.isEnabled) {
     ...
     * Validate URL
     */
    public validateUrl(url: string): { valid: boolean; error?: string } {
        if (!this.isEnabled) {
            return { valid: true };
        }
     ...
     */
    public validateInput(input: string, maxLength?: number): { valid: boolean; error?: string; sanitized?: string } {
        if (!this.isEnabled) {
     ...
     */
    public validatePackManifest(manifest: any): { valid: boolean; error?: string } {
        if (!this.isEnabled) {
            return { valid: true };
        }
    ...
    public getSecurityStats(): {
        ...
        rateLimitStats: { [key: string]: { count: number; resetTime: number } };
        isEnabled: boolean;
    } {
        const rateLimitStats: { [key: string]: { count: number; resetTime: number } } = {};
     ...
     */
    public logViolation(type: SecurityViolation['type'], message: string, source: string, data?: any): void {
