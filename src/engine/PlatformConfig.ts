...
export class PlatformManager {
    private static platforms: Record<string, PlatformConfig> = {
    ...
    private static currentPlatform: string = 'snes';

    public static getPlatform(key: string): PlatformConfig | null {
    ...
    public static getPlatformsByCategory(category: 'computer' | 'console' | 'handheld'): PlatformConfig[] {
        return Object.values(this.platforms).filter(p => p.category === category);
    }

    public static getCurrentPlatform(): PlatformConfig {
        return this.platforms[this.currentPlatform];
    }

    public static setCurrentPlatform(key: string): boolean {
    ...
    public static getPlatformKeys(): string[] {
        return Object.keys(this.platforms);
    }

    public static getPlatformNames(): string[] {
        return Object.values(this.platforms).map(p => p.name);
    }

    public static getPlatformByName(name: string): PlatformConfig | null {
        return Object.values(this.platforms).find(p => p.name === name) || null;
    }

    public static getPlatformsByYear(year: number): PlatformConfig[] {
        return Object.values(this.platforms).filter(p => p.year === year);
    }

    public static getPlatformsByResolution(width: number, height: number): PlatformConfig[] {
        return Object.values(this.platforms).filter(p => 
            p.resolution.width === width && p.resolution.height === height
        );
    }

    public static getUniqueResolutions(): Array<{width: number, height: number, platforms: PlatformConfig[]}> {
        const resolutionMap = new Map<string, {width: number, height: number, platforms: PlatformConfig[]}>();
        ...
        return Array.from(resolutionMap.values()).sort((a, b) => 
