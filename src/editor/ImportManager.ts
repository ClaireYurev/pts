...
export class ImportManager {
  ...
  private supportedFormats = ['.json', '.ptspack.json', '.ptspack'];
  ...
  private parsePackData(content: string): any {
    try {
  ...
  private validatePackStructure(packData: any): { valid: boolean; errors: string[]; warnings: string[] } {
    const errors: string[] = [];
  ...
  private loadPackIntoEditor(packData: any): void {
    try {
      // Load level data
      if (packData.level) {
        if (packData.level.tileMap) {
      ...
      if (packData.scripts) {
        if (Array.isArray(packData.scripts.rules)) {
          ...
          const scriptEditor = (this.editor as any).scriptEditor;
        ...
        if (Array.isArray(packData.scripts.cutscenes)) {
          ...
          const cutsceneEditor = (this.editor as any).cutsceneEditor;
  ...
  private async loadDataUrlAsset(dataUrl: string): Promise<void> {
    return new Promise((resolve, reject) => {
      const img = new Image();
      img.onload = () => {
        // Add to tile palette if it's a tileset
        const tilePalette = (this.editor as any).tilePalette;
  ...
  public async loadFromUrl(url: string): Promise<ImportResult> {
    try {
  ...
  public validatePackCompatibility(packData: any): { compatible: boolean; issues: string[] } {
    const issues: string[] = [];
  ...
  private checkMissingAssets(assets: string[]): string[] {
  ...
  public createImportPreview(packData: any): string {
    const preview = {
  ...
  private countTiles(tileMap: number[][]): number {
    if (!tileMap || !Array.isArray(tileMap)) return 0;
