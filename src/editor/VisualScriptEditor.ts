import { ECAScript, ECANode, ECAEdge } from '../runtime/scripting/ECA';
...
export class VisualScriptEditor {
  ...
  private connectingFrom: { node: ECANode; port: string } | null = null;
  ...
  private createEmptyScript(): ECAScript {
    return {
      id: 'script_' + Date.now(),
      name: 'New Script',
      nodes: [],
  ...
  private addNode(template: NodeTemplate, x: number, y: number) {
    const node: ECANode = {
  ...
  setScript(script: ECAScript) {
    this.script = script;
    this.render();
  }

  addEdge(fromNodeId: string, fromPort: string, toNodeId: string, toPort: string) {
    const edge: ECAEdge = {
  ...
  removeNode(nodeId: string) {
    ...
    this.script.edges = this.script.edges.filter(e => 
